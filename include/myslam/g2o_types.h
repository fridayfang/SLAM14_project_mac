...
class VertexPose : public g2o::BaseVertex<6, SE3> {
   public:
    ...
    virtual void setToOriginImpl() override { _estimate = SE3(); }//重载:setToOriginImpl

    /// left multiplication on SE3
    virtual void oplusImpl(const double *update) override {//重载:update 是扰动，定义扰动后的变化
        Vec6 update_eigen;
...
class EdgeProjectionPoseOnly : public g2o::BaseUnaryEdge<2, Vec2, VertexPose> {
   public:
    ...
    virtual void computeError() override {
        ...
        _error = _measurement - pos_pixel.head<2>();//地图点投影到像素坐标的误差　_measurement 通过setMeasurement 初始化
    }

    virtual void linearizeOplus() override {
        const VertexPose *v = static_cast<VertexPose *>(_vertices[0]);
        SE3 T = v->estimate();// return _estimate
